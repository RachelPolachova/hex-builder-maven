...
//    }
    ...
    private Double euclideanDistance(LocationCoordinate2D a, LocationCoordinate2D b) {
        return Math.sqrt(Math.pow(a.getX() - b.getX(), 2) + Math.pow(a.getY() - b.getY(), 2));
    }

    private ArrayList<LocationCoordinate2D> kNearestNeighbors(ArrayList<LocationCoordinate2D> l, LocationCoordinate2D q, Integer k) {
        ArrayList<Pair<Double, LocationCoordinate2D>> nearestList = new ArrayList<>();
        for (LocationCoordinate2D o : l) {
            nearestList.add(new Pair<>(euclideanDistance(q, o), o));
        }

        Collections.sort(nearestList, new Comparator<Pair<Double, LocationCoordinate2D>>() {
            @Override
            public int compare(Pair<Double, LocationCoordinate2D> o1, Pair<Double, LocationCoordinate2D> o2) {
                return o1.getKey().compareTo(o2.getKey());
            }
        });

        ArrayList<LocationCoordinate2D> result = new ArrayList<>();
    ...
    private LocationCoordinate2D findMinYPoint(ArrayList<LocationCoordinate2D> l) {
        Collections.sort(l, new Comparator<LocationCoordinate2D>() {
            @Override
            public int compare(LocationCoordinate2D o1, LocationCoordinate2D o2) {
    ...
    private Double calculateAngle(LocationCoordinate2D o1, LocationCoordinate2D o2) {
    ...
    private ArrayList<LocationCoordinate2D> sortByAngle(ArrayList<LocationCoordinate2D> l, LocationCoordinate2D q, Double a) {
        // Sort by angle descending
        Collections.sort(l, new Comparator<LocationCoordinate2D>() {
            @Override
            public int compare(final LocationCoordinate2D o1, final LocationCoordinate2D o2) {
    ...
    private Boolean intersect(LocationCoordinate2D l1p1, LocationCoordinate2D l1p2, LocationCoordinate2D l2p1, LocationCoordinate2D l2p2) {
    ...
    private boolean pointInPolygon(LocationCoordinate2D p, ArrayList<LocationCoordinate2D> pp) {
    ...
    public ConcaveHullAch() {

    }

    public ArrayList<LocationCoordinate2D> calculateConcaveHull(ArrayList<LocationCoordinate2D> pointArrayList, Integer k) {

        // the resulting concave hull
        ArrayList<LocationCoordinate2D> concaveHull = new ArrayList<>();

        // optional remove duplicates
        HashSet<LocationCoordinate2D> set = new HashSet<>(pointArrayList);
        ArrayList<LocationCoordinate2D> pointArraySet = new ArrayList<>(set);
        ...
        LocationCoordinate2D firstPoint = findMinYPoint(pointArraySet);
        concaveHull.add(firstPoint);
        LocationCoordinate2D currentPoint = firstPoint;
        ...
        while ((currentPoint != firstPoint || step == 2) && pointArraySet.size() > 0) {
            ...
            ArrayList<LocationCoordinate2D> kNearestPoints = kNearestNeighbors(pointArraySet, currentPoint, kk);

            // sort points by angle clockwise
            ArrayList<LocationCoordinate2D> clockwisePoints = sortByAngle(kNearestPoints, currentPoint, previousAngle);
